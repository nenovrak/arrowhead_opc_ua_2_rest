...
public class FullProviderMain extends ArrowheadClientMain {
  ...
  private FullProviderMain(String[] args) {
    Set<Class<?>> classes = new HashSet<>(Arrays.asList(OPCValueResource.class));
  ...
  private void loadAndCompilePayloads(boolean fromFile) {
    if (fromFile) {
      ...
      if (NEED_ORCH) {
        storeEntry = Collections.singletonList(new OrchestrationStore(service, consumer, provider, 1, false));
      }
    }
