...
public class OPC2RESTConsumer {
    ...
    private OPC2RESTConsumer(String[] args) throws InterruptedException {
        ...
        for (int i = 0; i < 100; i++) {
            Thread.sleep(200);
            OPCVariableReadout opcVar = consumeService(providerUrl);
            if (opcVar != null) {
                System.out.printf("Variable : %s:%s\n", opcVar.getType(), opcVar.getValue());
            }
        }
    ...
    public static void main(String[] args) {
        try {
            new OPC2RESTConsumer(args);
        } catch (InterruptedException ex) {
            Logger.getLogger(OPC2RESTConsumer.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    ...
    private ServiceRequestForm compileSRF() {
      ...
      SystemName can be an arbitrarily chosen name, which makes sense for the use case.
         */
        ArrowheadSystem consumer = new ArrowheadSystem(consumerSystemName, props.getProperty("consumer_adress"), 8080, null);
      ...
      so the communication between consumer and provider can be facilitated.
         */
        ArrowheadService service = new ArrowheadService("OPCUARead", Collections.singleton("json"), metadata);
    ...
    private OPCVariableReadout consumeService(String providerUrl) {
        /*
      ...
      Supported method types at the moment: GET, POST, PUT, DELETE
         */
        Response getResponse = Utility.sendRequest(providerUrl + "/10.48.134.10:4840/3/%22xBG5%22", "GET", null);
      ...
      .common.exception.ErrorMessage class, and the errors from the Orchestrator are parsed this way.
         */
        OPCVariableReadout readout = new OPCVariableReadout();
        try {
            readout = getResponse.readEntity(OPCVariableReadout.class);
